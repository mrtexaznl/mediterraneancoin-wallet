...
public final class WalletActivity extends AbstractOnDemandServiceActivity
{
	...
	private static final int DIALOG_PRECISION_WARNING = 2;
	...
	private void handleIntent(@Nonnull final Intent intent)
	{
		...
		if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(action))
		{
			...
			{
				@Override
				protected void bitcoinRequest(final PaymentIntent paymentIntent)
				{
	...
	private void handleDonate()
	{
		try
		{
			SendCoinsActivity.start(this, new PaymentIntent(Constants.DONATION_ADDRESS, getString(R.string.wallet_donate_address_label)));
		}
	...
	protected Dialog onCreateDialog(final int id)
	{
		...
		else if (id == DIALOG_PRECISION_WARNING)
			return createPrecisionWarningDialog();
		else
	...
	private Dialog createPrecisionWarningDialog()
	{
		final AlertDialog.Builder builder = new AlertDialog.Builder(this);
		...
		builder.setTitle("Important news");
		builder.setMessage("Bitcoin denomination has been changed from BTC to mBTC. All amounts are multiplied by a factor of 1000.\n\nYou can switch back to BTC in the settings.");
	...
	private void checkAlerts()
	{
		...
		if (config.hasBtcPrecision() && config.versionCodeCrossed(application.packageInfo().versionCode, DEFAULT_PRECISION_CHANGE_VERSION_CODE))
		{
			showDialog(DIALOG_PRECISION_WARNING);
		}
